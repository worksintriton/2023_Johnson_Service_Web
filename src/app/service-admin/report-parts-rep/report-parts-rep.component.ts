...
@Component({
  selector: 'app-report-parts-rep',
  templateUrl: './report-parts-rep.component.html',
  styleUrls: ['./report-parts-rep.component.css']
})
export class ReportPartsRepComponent implements OnInit {
...
var year = new Date(new Date().getFullYear() , 0, 1);
    ...
    if(this.Admin_check == null){
      ...
      this._api.pack_chart(this.data).subscribe((response: any) => {
...
this._api.pack_chart(this.data).subscribe((response: any) => {
 ...
 clickdata(e:any,data) {
  ...
  sessionStorage.setItem("chart_page",'partsrep');
