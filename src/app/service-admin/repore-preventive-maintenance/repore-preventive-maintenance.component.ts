...
@Component({
  selector: 'app-repore-preventive-maintenance',
  templateUrl: './repore-preventive-maintenance.component.html',
  styleUrls: ['./repore-preventive-maintenance.component.css']
})
export class ReporePreventiveMaintenanceComponent implements OnInit {
...
var year = new Date(new Date().getFullYear() , 0, 1);
    ...
    if(this.Admin_check == null){
      ...
      this._api.preventive_chart(this.data).subscribe((response: any) => {
...
this._api.preventive_chart(this.data).subscribe((response: any) => {
 ...
 clickdata(e:any,data) {
  ...
  sessionStorage.setItem("chart_page",'preventive');
